...
// CheckClock refuses an admission time earlier than the boundaries the
// import's sessions are admitted across: the current destination, and each
// configured project they land in. A clock set back would otherwise admit
// sessions the configuration then rejects, silently.
