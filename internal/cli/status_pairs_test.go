...
func publishPairSession(t *testing.T, home string, store *state.Store, remote *storage.MemoryStore, cfg config.Config, now time.Time, id, harness, project string, verify bool) {
	t.Helper()
