...
// when verify is set.
func publishPairSession(t *testing.T, home string, store *state.Store, remote *storagetest.MemoryStore, cfg config.Config, now time.Time, id, project string, verify bool) {
	t.Helper()
