...
func Run(ctx context.Context, local *LocalStore, store storage.ObjectStore, opts Options) (_ Result, runErr error) {
	...
	for i, reg := range registrations {
		...
		if req.Token == "" || reg.SourceKind == archive.SourceKindCursorSQLite {
			...
			case failure != "":
				...
				fail(unchangedSinceFailureError{message: failure})
				continue
			default:
	...
	status := Status{
		...
		LastError:              lastError,
		QuarantinedFiles:       local.quarantinedFiles(),
		UnrefreshableSummaries: local.countRefreshSkips(opts.parserVersion()),
	}
