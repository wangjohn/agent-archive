...
// versions so a reader can tell which policy produced it. (Filter 8 and adapter
// 0.8.0 superseded the filter-3 to filter-7 values this test pinned before.)
func TestFilterVersionIsDeclaredInCaptureProvenance(t *testing.T) {
	if FilterVersion != "8" || adapterVersion != "0.8.0" {
