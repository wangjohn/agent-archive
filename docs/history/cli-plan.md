...
3. **Review and start.** Show friendly application names, exact paths, destination, session scope, and automatic deletion after 90 days by default. Explain best-effort filtering and unknown bucket privacy. Ask “Start archiving?” as a numbered menu: start, edit, or cancel. Every multiple-choice prompt in setup is a numbered menu answered by number, with Enter taking the bracketed default. Edit exposes apps, projects, sessions, retention, storage, folder, and AWS region. Recheck changed storage before applying; do not repeat the probe for other edits. Require confirmation before activating capture.
