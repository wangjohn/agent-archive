...
- `repo_key`: `sha256(normalized origin remote URL)[:16]`, as already proposed in the [handoff specification](agent-archive-handoff-spec.md). It is written for local and cloud sessions alike, so the same repository groups together even though `project_id` (a path hash) differs between a Mac and a VM.
