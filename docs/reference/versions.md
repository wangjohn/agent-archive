...
| Filter | `archive.FilterVersion` | source header `capture.filter_version`, metadata `filter_version` | 8 | What the privacy filter keeps, drops, or redacts changes: any change to filtered output. |
| Adapter | `adapterVersion` in `internal/archive/adapters.go` | `capture.adapter_version` | 0.8.0 | An adapter's output changes (bumped with the filter in practice). |
| Parser | `archive.DefaultParserVersion` | metadata `parser.version` | 0.9.0 | How metadata is derived from a source changes: counts, turns, models, skills, gaps. |
